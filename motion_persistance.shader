...
uniform float depthWeight < string label="Depth Influence on Trails"; string widget_type="slider"; float minimum=0.0; float maximum=1.0; float step=0.001; > = 0.3;
uniform float frameTime   < string label="Frame Time (s)"; string widget_type="slider"; float minimum=0.0; float maximum=0.1; float step=0.001; > = 0.0;
...
float4 mainImage(VertData v_in) : TARGET
{
    ...
    float4 prev = previous_output.Sample(textureSampler, uv);
    ...
    float lumH = dot(image.Sample(textureSampler, uv + float2(px.x,0)).rgb - image.Sample(textureSampler, uv - float2(px.x,0)).rgb, LUMA);
    float lumV = dot(image.Sample(textureSampler, uv + float2(0,px.y)).rgb - image.Sample(textureSampler, uv - float2(0,px.y)).rgb, LUMA);
    float2 blurDir = -normalize(float2(lumH, lumV) + 1e-6);

    // ---------------- Hardcoded 7 Subsamples with normalized weights ----------------
    float4 accum =
